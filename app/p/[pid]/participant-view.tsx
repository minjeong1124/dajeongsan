...
export function ParticipantView() {
    ...
    return (
      <div className="space-y-5">
        <header className="animate-fade-in-up pt-6">
          <p className="text-3xl">👋</p>
          <h1 className="mt-3 text-2xl font-extrabold text-[#4A3728]">
            처음 접속한 기기 같아요!
          </h1>
        ...
        <section
          className="animate-fade-in-up space-y-2 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-[#F0E6D2]"
          style={{ animationDelay: "80ms" }}
        >
          <label htmlFor="last4" className="text-sm font-semibold text-[#6A5443]">
            계좌번호 뒤 4자리
          </label>
          <input
            id="last4"
            ...
            className="tnum h-12 w-full rounded-xl border border-[#E8DCC5] bg-white px-4 text-base tracking-widest outline-none transition-all focus:border-[#8FA9DA] focus:ring-2 focus:ring-[#E3EBF8]"
          />
        ...
        <div className="animate-fade-in-up space-y-3" style={{ animationDelay: "160ms" }}>
          ...
          <button
            ...
            className="h-12 w-full rounded-xl border border-[#E8DCC5] bg-white text-base font-semibold text-[#77614E] transition-all hover:border-[#AFC3E8] active:scale-95"
          >
            건너뛰기
          </button>
          <p className="text-center text-xs text-[#A3927E]">
  ...
  return (
    <div className="space-y-5">
      <header className="animate-fade-in-up pt-6">
        {cutApplied ? (
          <>
            ...
            <p className="mt-1.5 text-sm font-medium text-[#5F82C2]">
              끝자리는 요청자가 덜어줬어요!
            </p>
          </>
        ) : (
      ...
      <section
        className="animate-fade-in-up rounded-2xl bg-white p-5 shadow-sm ring-1 ring-[#F0E6D2]"
        style={{ animationDelay: "80ms" }}
      >
        {cutApplied ? (
          <>
            <p className="tnum text-base text-[#A3927E] line-through">
              {formatWon(participant.baseAmount)}
            </p>
            <div className="mt-1">
              <CountUpAmount from={participant.baseAmount} to={finalAmount} />
            </div>
            <p className="tnum mt-2 text-sm font-semibold text-[#5F82C2]">
          ...
          <>
            ...
            <p className="mt-2 text-sm text-[#8C7963]">아래 계좌로 직접 송금해주세요!</p>
          </>
        )}
      </section>
      ...
      >
        <p className="text-xs font-semibold text-[#A3927E]">보낼 계좌</p>
        {(() => {
          const bank = getBank(request.requesterBankCode);
          return (
            <div className="mt-1.5 flex items-center gap-2">
              {bank && <BankLogo bank={bank} size={28} />}
              <p className="break-all text-base font-bold text-[#4A3728]">
                {bank ? `${bank.name} ` : ""}
                {request.requesterAccount}
              </p>
            </div>
        ...
        <p className="mt-1 text-xs text-[#A3927E]">예금주: {request.requesterName}</p>
        <button
          type="button"
          onClick={copyAccount}
      ...
      <p
        className="animate-fade-in-up text-center text-xs text-[#A3927E]"
